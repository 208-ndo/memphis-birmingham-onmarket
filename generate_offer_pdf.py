...
def generate_offer_pdf(listing: dict, offer: dict, output_dir: str = 'data') -> str:
    ...
    agent_commission = round(list_price * 0.05)
    ...
    addr_data = [[
        [Paragraph(address, addr_style),
         Paragraph(f'{city}, {state}  |  Listed: {format_currency(list_price)}  |  DOM: {listing.get("days_on_market", 0)} days', sub_style)],
    ...
    commission_data = [[
        Paragraph(
            f'<b><font color="#ffffff">🎯  AGENT COMMISSION FULLY PROTECTED</font></b>',
            ParagraphStyle('comm', fontSize=11, fontName='Helvetica-Bold',
                         textColor=white, leading=14)
        ),
        Paragraph(
            f'<b><font color="#00d4aa">{format_currency(agent_commission)}</font></b>'
            f'<font color="#6b6b8a">  paid at closing</font>',
            ParagraphStyle('commAmt', fontSize=13, fontName='Helvetica-Bold',
    ...
    of_content = [
        ...
        Spacer(1, 6),
        Paragraph(
            '<font color="#6c63ff">★ Seller gets near full asking price</font>',
    ...
    elements.append(Paragraph(
        '<font color="#6b6b8a">Closing Costs: Buyer will pay all closing costs minus any unpaid mortgages, taxes, or liens. '
        'Buyer reserves right to assign contract. Closing location selected by Buyer.</font>',
    ...
    footer_data = [[
        [
            Paragraph('<b>Michael</b>', ParagraphStyle('sig', fontSize=11,
                     fontName='Helvetica-Bold', textColor=COLOR_DARK, leading=14)),
            Paragraph('229 Holdings LLC', ParagraphStyle('sigco', fontSize=9,
                     fontName='Helvetica', textColor=COLOR_LIGHT_TEXT, leading=12)),
